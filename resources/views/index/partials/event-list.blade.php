                    ...
                    @foreach ($events as $event)
                        <div class="w-full sm:w-44 md:w-56 lg:w-[19rem] xl:w-96  lg:h-96 xl:h-96 sm:rounded-2xl  text-white text-center font-black bg-gray-600 mt-10 sm:mt-0 px-3 pt-4  sm:py-5 md:pt-0 pb-3 lg:pb-0">
                            ...
                            @if($event->registration_status == 0)
                                <a class="w-full inline-block text-center hover:bg-red-700 hover:text-white rounded  bg-red-400 text-white md:text-xl px-4 py-6 sm:py-2  mt-5 sm:mt-5" href="{{ route('registration.create',$event->id) }}">Registrace</a>
                            @else
                                <a class="w-full inline-block text-center hover:bg-red-700 hover:text-white rounded  bg-green-400 text-white md:text-xl px-4 py-6 sm:py-2 mt-5 sm:mt-5" href="{{ route('event.upload-url.create',$event->id) }}">Nahrát běh manuálně</a>
                            @endif
